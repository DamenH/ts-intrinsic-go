//// [tests/cases/compiler/intrinsic/customIntrinsics.ts] ////
...
const myRange = (n) => { let arr = []; let i = 0; while (i < n) {
    arr = [...arr, i];
    i = i + 1;
} return arr; };
// --- Higher-order: map, filter, reduce ---
const keepPositive = (t) => t.filter((x) => x > 0);
const doubled = (t) => t.map((x) => x * 2);
const sum = (t) => t.reduce((a, b) => a + b, 0);
const unique = (t) => t.filter((x, i) => t.indexOf(x) == i);
// --- Control flow ---
const factorial = (n) => {
    let result = 1;
    let i = n;
...
// Deep property access: reduce over a split path
const deepGet = (t, path) => path.split('.').reduce((obj, key) => obj[key], t);
