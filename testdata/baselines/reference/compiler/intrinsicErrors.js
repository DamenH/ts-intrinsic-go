...
//// [intrinsicErrors.js]
"use strict";
const NeverType = Symbol.for("tsi.never");
// switch: not supported
const withSwitch = (x) => {
    switch (typeof x) {
        case 'string': return x;
...
// optional chaining: not supported
const withOptional = (x) => x?.name;
// destructured params: not supported
const withDestructure = ({ name, age }) => name;
// === (only == is supported)
const withStrict = (x) => x === 'hello' ? x : NeverType;
// nullish coalescing: not supported
const withNullish = (x) => x ?? 'default';
// template literal: not supported
const withTemplate = (name) => `hello ${name}`;
// runtime error: division by zero produces Infinity, not an error
const divByZero = (n) => n / 0;
// runtime error: property access on number
const badAccess = (n) => n.foo;
// budget exceeded: infinite loop
const infinite = (n) => {
    let i = 0;
    while (i < n) {
