//// [tests/cases/compiler/intrinsic/intrinsicDeps.ts] ////
...
// Same-file dependency resolution for intrinsic functions.
const double = (n) => n * 2;
const useDouble = (x) => double(x) + 1;
// Transitive: add4 -> add2 -> add1
const add1 = (n) => n + 1;
const add2 = (n) => add1(add1(n));
const add4 = (n) => add2(add2(n));
// Shared helper
const normalize = (s) => s.trim().toLowerCase();
const greet = (name) => "hello " + normalize(name);
const shout = (name) => normalize(name).toUpperCase();
// Const value dependency
const PREFIX = "test_";
const withPrefix = (s) => PREFIX + s;
// Multiple helpers
const square = (n) => n * n;
const negate = (n) => 0 - n;
const negSquare = (x) => negate(square(x));
// Helper referenced inside a closure
const isEven = (n) => n % 2 == 0;
const filterEvens = (arr) => arr.filter((x) => isEven(x));
// Object return
const makePoint = (x, y) => {
    let result = {};
    result["x"] = x;
...
const shiftPoint = (x, y, dx, dy) => makePoint(x + dx, y + dy);
// Diamond: combined -> doubleInc -> inc, combined -> tripleInc -> inc
const inc = (n) => n + 1;
const doubleInc = (n) => inc(n) * 2;
const tripleInc = (n) => inc(n) * 3;
const combined = (n) => doubleInc(n) + tripleInc(n);
// Helper calling helper
const capitalize = (s) => {
    if (s.length == 0)
        return s;
...
const titleCase = (s) => s.split(" ").map((w) => capitalize(w)).join(" ");
// Comments in function bodies
const withLineComments = (n) => {
    // double it
    let x = n * 2;
    // then add one
    return x + 1;
};
const withBlockComments = (n) => {
    let x = n /* input */ + 1;
    /* square */
    return x * x;
};
const commented_helper = (s) => {
    // trim and lowercase
    return s.trim().toLowerCase();
};
const use_commented = (s) => commented_helper(s) + "!";
