...
//// [intrinsicEdgeCases.js]
"use strict";
// --- Circular / self-referencing type arguments ---
const identity = (x) => x;
// --- Deeply nested closures ---
const nestedClosures = (n) => {
    let a = (x) => {
        let b = (y) => {
            let c = (z) => {
                let d = (w) => w + z + y + x;
...
// --- Closing over top-level const values ---
const MULTIPLIER = 3;
const scaleUp = (n) => n * MULTIPLIER;
const OFFSET = 100;
const addOffset = (n) => n + OFFSET;
const GREETING = "hi";
const greetWith = (name) => GREETING + " " + name;
// --- Union types as arguments ---
const double = (n) => n * 2;
...
const needsTwo = (a, b) => a + b;
// --- Semantic mismatches that should stay explicit ---
const kindOf = (x) => typeof x;
const kindOfLocalFn = () => {
    let f = (x) => x + 1;
    return typeof f;
};
