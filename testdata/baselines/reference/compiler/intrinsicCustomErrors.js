...
//// [static_errors.js]
"use strict";
const _requireString = (x) => {
    if (typeof x != 'string')
        return void { error: "Expected a string" };
    return x;
};
//// [dynamic_errors.js]
"use strict";
const _inRange = (n, lo, hi) => {
    if (typeof n != 'number')
        return void { error: "Expected a number, got " + typeof n };
    if (n < lo)
        return void { error: n + " is below minimum " + lo };
    if (n > hi)
        return void { error: n + " is above maximum " + hi };
    return n;
};
//// [enum_errors.js]
"use strict";
const _httpMethod = (m) => {
    if (typeof m != 'string')
        return void { error: "HTTP method must be a string" };
    let upper = m.toUpperCase();
    if (upper != 'GET' && upper != 'POST' && upper != 'PUT' && upper != 'DELETE' && upper != 'PATCH') {
        return void { error: "Unknown HTTP method '" + m + "'. Expected GET, POST, PUT, DELETE, or PATCH" };
    }
    return upper;
...
//// [lib_validators.js]
export const _hexColor = (s) => {
    if (typeof s != 'string')
        return void { error: "Hex color must be a string, got " + typeof s };
    if (s.length != 7)
        return void { error: "Hex color must be 7 characters (e.g. '#ff0000'), got " + s.length };
    if (s.charAt(0) != '#')
        return void { error: "Hex color must start with '#', got '" + s + "'" };
    return s;
};
//// [use_lib.js]
export {};
...
const _parseAge = (x) => {
    if (typeof x != 'number')
        return void "never";
    if (x < 0)
        return void { error: "Age cannot be negative (" + x + ")" };
    if (x > 150)
        return void { error: "Age " + x + " is unrealistic (max 150)" };
    if (Math.floor(x) != x)
        return void { error: "Age must be a whole number, got " + x };
    return x;
};
//// [object_errors.js]
"use strict";
const _config = (cfg) => {
    if (typeof cfg != 'object')
        return void { error: "Config must be an object" };
    if (typeof cfg['host'] != 'string')
        return void { error: "Config.host must be a string" };
    if (cfg['host'].length == 0)
        return void { error: "Config.host must not be empty" };
    if (typeof cfg['port'] != 'number')
        return void { error: "Config.port must be a number" };
    if (cfg['port'] < 1 || cfg['port'] > 65535) {
        return void { error: "Config.port " + cfg['port'] + " is out of range (1-65535)" };
    }
    return cfg;
